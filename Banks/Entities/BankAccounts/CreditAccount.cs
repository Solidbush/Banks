...
public class CreditAccount : IBankAccount
{
    ...
    public CreditAccount(int id, Client client, string accountName, decimal creditLimit, decimal transferLimit, decimal interestRate)
    {
        ...
        if (interestRate < MinInterestRate)
            throw new Exception();
        if (creditLimit < InitialConditions)
            throw new Exception();
        Id = id;
        Client = client;
    ...
    {
        if (_balance - Math.Abs(money) < -_creditLimit) 
            throw new Exception();
        _balance -= Math.Abs(money);
    }
    ...
    {
        if (((_balance - Math.Abs(money) - commission) < _creditLimit) ||
    ...
    {
        if (newCreditLimit < InitialConditions)
            throw new Exception();
